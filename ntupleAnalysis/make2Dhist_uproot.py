...
masspoints = ['3p7','4','5','6','8','10','12','14']

granularity = 'HCAL'

for mass in masspoints:
    infile = uproot.open(f'/afs/cern.ch/work/r/rchudasa/private/TauClassification/run3/CMSSW_13_0_17/src/Gen/GenInfo_only_H2AA4Tau_M{mass}GeV.root')
    print(infile)

    rhTree = infile['fevt/RHTree']
    deta_Tau  = rhTree['Tau1_Tau2_deta'].array()
    dphi_Tau  = rhTree['Tau1_Tau2_dphi'].array()
    deta_Tau2  = rhTree['Tau3_Tau4_deta'].array()
    dphi_Tau2  = rhTree['Tau3_Tau4_dphi'].array()

    if granularity == 'ECAL':
        h2 = Hist.new.Reg(10, 0, 0.174, name="deta").Reg(10, 0, 0.174, name="dphi").Double()
    if granularity == 'HCAL':
        h2 = Hist.new.Reg(10, 0, 0.87, name="deta").Reg(10, 0, 0.87, name="dphi").Double()
    h2.fill(deta=ak.flatten(deta_Tau), dphi=ak.flatten(dphi_Tau))
    h2.fill(deta=ak.flatten(deta_Tau2), dphi=ak.flatten(dphi_Tau2))
    ...
    plt.title(f'H->AA->4Tau A ({mass} GeV) in {granularity} Granularity')
    plt.savefig(f'2dfigures/2d_plot_{mass}_{granularity}.png')
